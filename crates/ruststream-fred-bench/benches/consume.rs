...
//! Consuming a small JSON body from a stream: a consumer-group read yields this crate's message,
//! the dispatcher decodes it into a struct, the handler reads a field, and the runtime acks it
//! with an `XACK`.
...
// Twice MESSAGES deliveries allocated 59,193 to 59,196 blocks over five runs, the client's
...
#[library_benchmark(config = common::config_every(29_598, 1_000, 60))]
#[bench::first(app(1))]
