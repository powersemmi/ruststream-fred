# Contributing to ruststream-fred

`ruststream-fred` is the Redis and Valkey broker crate of RustStream. The framework's core crate lives in
...
git clone https://github.com/powersemmi/ruststream-fred.git
```
...
The `redis-stream`, `redis-pubsub` and `redis-list` scaffolds under `templates/` are rendered and
compiled by CI's lint job whenever they or the crate change, patched to this branch and to the
core's `main`. `cargo generate --path templates/redis-stream --name smoke` renders one locally.

`just bench` measures what this crate and the framework's runtime cost over the raw `fred` client
on the same stand and rewrites `docs/benchmarks/results.json`. It takes minutes and wants the
machine to itself. `just bench-code` counts what this crate's own code costs per message, in
instructions and allocations, on the production broker against the same stand, and rewrites the
code table of the same document.
...
From the core, `just brokers fred` runs the same command against the core's working tree.
