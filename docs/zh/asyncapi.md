...
`PUBLISH` 发出时用的投递模式、列表推入时重新设上的过期时间、它写出的信封。它还用 Redis 自己的说法
报告消息落在哪里：写入流或列表的发布者报告 `key`，Pub/Sub 的发布者报告 `channel`。这个名字就是
挂载点解析出来的目的地，所以用 `publish("orders.done")` 声明的回复，无论由哪个订阅产生，报告的
都是 `orders.done`。
