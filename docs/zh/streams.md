...
该模式的重试计数：[在挂载点声明的上限](dead-letter.md)读的就是它，而想自己拿主意的处理器读消息头：
...
- **已排期的延迟重试。** 已经躺在 ZSET 延迟队列里的副本以到期时间为分值，因此时间一到就追加回流里，
  与组读到哪里无关。
- **投递次数。** 重放的条目会再投递一次，它自带的投递次数因此增长；回收订阅于是把重放算进声明的
  上限，而框架的重试次数消息头只在真正的重试上才增加。
...
claiming 订阅改走待处理条目列表来重试：重试既不追加也不确认，等条目空闲满 `min_idle` 之后，这个订
阅自己的下一次读取把它取回来。没有重复，条目也保留自己的标识，代价是下一次
尝试要等过这个阈值。

## 延迟重试 { #delayed-retry }

`HandlerOutcome::retry_after(delay)` 请求一次延迟的重新投递，用来给暂时性的错误退避。Redis Streams
没有逐条消息的延迟，因此一个订阅通过三种方式之一来承担它。

运行时自己的方式是：延迟到期时，把这条消息的一份副本发布回流的键上：

```rust
--8<-- "crates/ruststream-fred/examples/fred_delayed_retry.rs:deferred"
```

这份副本要走的发布者，每个注册本来就有，取自 Broker 自己的默认发布策略，因此挂载点上什么都不用
写。副本在整个延迟窗口里是至多一次：定时器触发前崩溃就会丢掉它，而副本的重试次数消息头比原件
高一。
...
claiming 模式是第三种方式，它不需要队列：条目留在待处理条目列表里，这个订阅自己的下一次读取把它取
回来。它能给出的等待只有一个，就是自己的 `min_idle`，因此比它短的延迟向上取到它。想让延迟按写明的
时长生效，就在 claiming 订阅上也写明一个 ZSET 队列。

挂载点是替换副本发布者的地方，每个注册一次：

```rust
--8<-- "crates/ruststream-fred/examples/fred_delayed_retry.rs:app"
```

`out_retry(policy)` 给副本写明另一套策略，而这个位置是普通的 `Out` 槽位：它后面的 `.codec(..)` 或
`.transform(..)` 作用在每一份副本上。那里的变换读到的是正在被重试的那次投递，就像回复的变换读到
正在被回答的那次投递一样。
